...
from app.models.schemas import (
    VideoInfoRequest, VideoInfoResponse, DownloadRequest, DownloadResponse,
    DownloadRecordResponse, RetryDownloadRequest, SettingsUpdate, SettingsResponse
)
...
downloader_service = VideoDownloaderService(
    ...
    proxy_domains=settings.PROXY_DOMAINS,
...
async def get_video_info(request: VideoInfoRequest, db: Session = Depends(get_db)):
    ...
    existing = db.query(DownloadRecord).filter(DownloadRecord.url == request.url).order_by(DownloadRecord.created_at.desc()).first()
    if existing:
        logger.info(f"API: Found existing record - ID: {existing.id}, Status: {existing.status}")
        return VideoInfoResponse(
            id=existing.id,
            title=existing.title,
            description=existing.video_info.get("description") if existing.video_info else None,
            duration=existing.duration,
            uploader=existing.video_info.get("uploader") if existing.video_info else None,
            thumbnail=existing.video_info.get("thumbnail") if existing.video_info else None,
            formats=existing.video_info.get("formats", []) if existing.video_info else [],
...
async def start_download(request: DownloadRequest, db: Session = Depends(get_db)):
    ...
    logger.info(f"API: Start download request - URL: {request.url}, Quality: {request.quality}, Record ID: {request.record_id}")
    
    # If record_id provided, update existing record
    if request.record_id:
        record = db.query(DownloadRecord).filter(DownloadRecord.id == request.record_id).first()
        if record:
    ...
    asyncio.create_task(process_download(download_id, request.url, request.quality, record.id))
...
async def retry_download(request: RetryDownloadRequest, db: Session = Depends(get_db)):
    ...
    if record.status not in ["failed", "pending"]:
        ...
        raise HTTPException(status_code=400, detail=f"Cannot retry download with status: {record.status}")
    ...
    asyncio.create_task(process_download(download_id, record.url, request.quality, record.id))
...
async def process_download(download_id: str, url: str, quality: str, record_id: int):
    ...
    try:
        ...
        if result.get("success"):
            ...
            record.error_msg = None
            logger.info(f"API: Download completed - ID: {download_id}, File: {result.get('filename')}")
        else:
            record.status = "failed"
            record.error_msg = result.get("error", "Unknown error")
            logger.error(f"API: Download failed - ID: {download_id}, Error: {record.error_msg}")
...
async def get_downloads(limit: int = 50, db: Session = Depends(get_db)):
    ...
    records = db.query(DownloadRecord).order_by(DownloadRecord.created_at.desc()).limit(limit).all()
...
async def search_downloads(query: str, limit: int = 50, db: Session = Depends(get_db)):
    ...
    records = db.query(DownloadRecord).filter(
        or_(
            DownloadRecord.title.contains(query),
            DownloadRecord.url.contains(query)
        )
    ).order_by(DownloadRecord.created_at.desc()).limit(limit).all()
...
async def update_settings(settings_update: SettingsUpdate):
    ...
    if settings_update.proxy_domains is not None:
        settings.PROXY_DOMAINS = settings_update.proxy_domains
        ...
        while True:
            if download_id in active_downloads:
                ...
                if data["status"] in ["completed", "failed"]:
                    logger.info(f"API: WebSocket closing - Download {download_id} finished with status: {data['status']}")
                    break
